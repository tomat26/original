    ...
    UICollectionViewDelegate, UICollectionViewDelegateFlowLayout
{
    ...
    override func viewDidLoad() {
        ...
        collectionView.register(
            UINib(nibName: "PostCell", bundle: nil),
    ...
    func scheduleRandomDailyNotification() {
        ...
        let morningMessages = [
            "おはようございます！素晴らしい一日のスタートにミッションをどうぞ！",
            "素晴らしい1日のスタートを切るためのミッションをチェックしてみて！",
            "今日も一歩を踏み出すために、まずはミッションをクリアしよう！",
        ]
        let afternoonMessages = [
            "午後のスパイスに、楽しいチャレンジをどうぞ！", "忙しい日々の中でも、小さな楽しみを見つけましょう！",
            "午後のエネルギーをチャージして、ミッションを楽しんで！",
        ]
        let nightMessages = [
            "1日の締めくくりに、ちょっとしたミッションで達成感を感じてみよう！",
            "お疲れさまでした！今日もよく頑張ったね、ミッションで最後にもう一歩！", "1日を終える前に、ささやかな達成感を味わって！",
        ]
    ...
    func collectionView(
        ...
        cell.setupCell(
            profile: UIImage(
                named: posts[indexPath.row].profileImage
    ...
    func fetchPosts() {
        db.collection("posts").getDocuments { (querySnapshot, error) in
            ...
            self.posts =
                querySnapshot?.documents.compactMap { document -> Post? in
                    let data = document.data()

                    // 必要なフィールドを安全に取り出してPost型を生成
                    var id = data["id"] as? String ?? "defaultId"  // デフォルト値を設定
                    var profileImage =
                        data["profileImage"] as? String ?? "defaultprofileImage"  // デフォルト値を設定
                    var username =
                        data["username"] as? String ?? "defaultusername"  // デフォルト値を設定
                    var postImageURL =
                        data["postImages"] as? String ?? "No postImage"  // デフォルト値を設定
                    var comments = data["comments"] as? String ?? "No comments"  // デフォルト値を設定
                    var goodButton =
                        data["goodButton"] as? String ?? "No goodButton"  // デフォルト値を設定
            ...
            self.posts.sort { post1, post2 in
                guard let date1 = dateFormatter.date(from: post1.createdAt),
                    let date2 = dateFormatter.date(from: post2.createdAt)
                else {
